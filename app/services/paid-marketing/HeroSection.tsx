        ...
        <div className="w-full">
          <div className="grid grid-cols-2 md:grid-cols-5 max-w-full">
            {stats.map((stat, index) => (
              <div key={index} className={`py-6 relative px-6 md:px-8 ${index === 4 ? 'col-span-2 md:col-span-1' : ''}`}>
                <div className={`${index === 4 ? 'flex flex-col items-center md:items-start' : ''}`}>
                ...
                {index < stats.length - 1 && (
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 h-16 w-px bg-[#E5E7EB] hidden md:block"></div>
                )}
              </div>
            ))}
          </div>
